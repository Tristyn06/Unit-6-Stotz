...
public class ArrayLists {
    ...
    public static String findMin(ArrayList list){
        Person min = (Person) list.get(0);
        int age = min.getMyAge();
        for (int i = 0; i < list.size(); i++) {
            Person comp = (Person) list.get(i);
            if (comp.getMyAge() < age){
                min = comp;
                age = comp.getMyAge();
            }
        }
        return min.getMyName();
    }
